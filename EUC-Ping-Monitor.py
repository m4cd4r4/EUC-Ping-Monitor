# Macdara O Murchu
# 19.06.2024
# Ping multiple devices in the background (Windows only)
...
def check_ping(hostname, days_to_run, result_queue):
    ...
    for _ in range(max_checks):
        ...
        if response.returncode == 0:
            result_queue.put((hostname, response.stdout.decode('utf-8'), True))
            notifier.show_toast("Ping Success", f"{hostname} is now reachable", duration=10)
            return
        time.sleep(check_interval)

    result_queue.put((hostname, f"Could not reach {hostname} within {max_days} days", False))
    notifier.show_toast("Ping Timeout", f"Could not reach {hostname} within {max_days} days", duration=10)

def start_check_ping(hostnames, days_to_run, root, result_text):
    try:
    ...
    with ThreadPoolExecutor() as executor:
        for hostname in hostnames:
            executor.submit(check_ping, hostname.strip(), days_to_run, result_queue)
    
    root.after(100, lambda: process_results(result_queue, root, result_text))

def process_results(result_queue, root, result_text):
    while not result_queue.empty():
        hostname, result, success = result_queue.get()
        result_text.insert(tk.END, f"{hostname}:\n{result}\n")
...
def main():
    ...
    tk.Label(root, text="Hostnames (comma-separated):", bg='#F0F0F0', fg='#006AFF', font=('Arial', 12, 'bold')).grid(row=0, column=0, padx=10, pady=5, sticky='w')
    ...
    run_button = tk.Button(root, text="Run", 
                           command=lambda: start_check_ping(hostname_entry.get().split(','), days_entry.get(), root, result_text),
